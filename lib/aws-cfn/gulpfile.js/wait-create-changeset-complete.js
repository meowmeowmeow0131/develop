...
gulp.task('wait-create-changeset-complete', (callback) => {
                    ...
                    `
                        {
                            docker run --rm -i \
                                ...
                                -w /var/opt \
                                ${process.env.AWS_CLI_DOCKER_IMAGE} cloudformation wait change-set-create-complete \
                            ...
                            docker run --rm -i \
                                ...
                                -v ${config.workDir}:/var/opt \
                                -w /var/opt \
                                ${process.env.AWS_CLI_DOCKER_IMAGE} cloudformation describe-change-set \
                                    --change-set-name ${config.argv.CHANGE_SET_NAME} \
                                    --stack-name ${config.stackName} \
                                    --region ${process.env.AWS_REGION}
                            exit 1
